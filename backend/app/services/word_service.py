...
class WordService:
    ...
    def generate_report(self, 
        ...
        meta.add_run(f"Generated on {datetime.now().strftime('%B %d, %Y at %H:%M')}").italic = True
    ...
    def _extract_generic_data(self, content: str, keywords: List[str]) -> Dict[str, int]:
        ...
        for keyword in keywords:
            ...
            if matches:
                data[keyword] = int(matches[-1])  # Take last number (usually the total/count)
                continue
    ...
    def _add_chart_image(self, chart_data: Dict):
        ...
        try:
            colors = ['#e53e3e', '#ed8936', '#38a169', '#3182ce', '#805ad5', '#d69e2e', '#319795', '#dd6b20']
            ...
            if chart_data['type'] == 'pie':
                ...
                bars = ax.bar(x, chart_data['values'], color=colors[:len(chart_data['labels'])], edgecolor='white')
                ax.set_xticks(x)
                ...
                bars = ax.barh(y, chart_data['values'], color=colors[:len(chart_data['labels'])], edgecolor='white')
                ax.set_yticks(y)
