...
class PDFService:
    ...
    def generate_report(
        self, filename: Optional[str] = None, analysis_text: str = ""
    ) -> bytes:
        ...
        if filename:
            story.append(
                Paragraph(f"<b>Source:</b> {filename}", self.styles["CenteredBody"])
            )
        ...
        if charts:
            ...
            for chart_data in charts:
                ...
                if chart_img:
                    ...
                    story.append(
                        Paragraph(
                            f"<i>{chart_data.get('title', 'Chart')}</i>",
                            self.styles["CenteredBody"],
                        )
                    )
        ...
        try:
            ...
            if chart_data["type"] == "pie":
                ...
                bars = ax.bar(
                    x,
                    chart_data["values"],
                    color=color_list[: len(chart_data["labels"])],
                    edgecolor="white",
                )
                ax.set_xticks(x)
